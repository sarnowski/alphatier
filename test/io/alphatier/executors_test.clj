...
(deftest register-executor-test
  (let [pool (tools/create-test-pool)]
    ...
    (testing "new registration"
      ...
      (let [{:keys [executors]} (pools/get-snapshot pool)
...
(deftest update-instance-metadata-test
  (let [pool (tools/create-test-pool)
        [executor-id _] (-> pool pools/get-snapshot :executors first)
        task (tools/create-test-task executor-id)]

    (schedulers/commit pool (pools/map->Commit {:scheduler-id "test-scheduler"
